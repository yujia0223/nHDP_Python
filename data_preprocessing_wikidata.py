...
try:
    raw_data = pd.read_csv('data/wikidata5m_inductive/wikidata5m_inductive_train.txt', delimiter='\t', names=['Subject', 'Predicate', 'Object'])
    print('---------- Raw data loaded successfully ----------')

    subset_train = raw_data.sample(n=1000000)
    # Preprocessing
    indexed_corpus, vocab, new_data_df, all_subjects, subject_documents = preprocess(subset_train)
    ...
    timestamp = '20240414_train'

    # Define the file path
    file_path = f'data/wikidata5m_inductive/subject_documents_{timestamp}.pkl'
    ...
    filename = f'data/wikidata5m_inductive/corpus_po_{timestamp}.txt'
    ...
    vocab_filename = f'data/wikidata5m_inductive/vocab_po_{timestamp}.txt'
